...
describe("BreakdownTable", () => {
  it("renders one row per group with USD-labelled columns", () => {
    render(
      <BreakdownTable
        breakdown={{
          group_by: "country",
          ...
          rows: [
            {
              key: "United States",
              employee_count: 2570,
              total_payroll_usd: "306210500.00",
              average_salary_usd: "119147.28",
            },
    ...
    expect(screen.getByRole("rowheader", { name: "India" })).toBeInTheDocument();
    expect(screen.getByText("USD 306,210,500.00")).toBeInTheDocument();
    expect(screen.getByText("2,570")).toBeInTheDocument();
  });

  it("shows an empty state when there are no rows", () => {
    render(<BreakdownTable breakdown={{ group_by: "job_title", currency: "USD", rows: [] }} />);

    expect(screen.getByRole("heading", { name: "By job title" })).toBeInTheDocument();
